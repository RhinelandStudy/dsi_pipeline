...
cleanIfNeeded() {
    ...
    if [ "$DELETEFILES" = 1 ] ; then
        ...
        # If we are keeping the jobmanager database, then
        # we need to keep the security details too otherwise
        # we can't decrypt the jobs in the database.
        if [ "$PRESERVEJOBS" = 0 ] ; then
            . "$UTILBASE/deleteServiceFiles.sh"
...
usage()
{
    ...
    echo "            [ -clean | -cleanPreserveJobs ] [ -mdcedef mdce_defaults_file ]"
    echo "            [ -usemhlm ] [ -version ] [ -help ]"
    echo
    ...
    echo
    echo "-cleanPreserveJobs  Perform a cleanup of all service checkpointing and"
    echo "                    log files before starting the service, or after stopping it."
    echo "                    Any existing jobs and tasks will be preserved."
    echo "                    Note: This will delete all information about any job managers"
    echo "                    or workers this service has ever maintained."  
    echo
    ...
    echo
    echo "-usemhlm            Use with mdce start to use MathWorks Hosted License Manager for"
    echo "                    workers. This overrides any value defined in the mdce defaults"
    echo "                    file."
    echo
